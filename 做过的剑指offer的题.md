...
[51.正则表达式匹配](#51正则表达式匹配)  
[52.表示数值的字符串](#####52表示数值的字符串)  
[53.字符流中第一个不重复的数字](#####53字符流中第一个不重复的数字)  
[54.链表中环的入口节点](#####54链表中环的入口节点)  
[55.删除链表中重复的元素](#####55删除链表中重复的元素)  
[56.二叉树的下一个节点](#####56二叉树的下一个节点)  
[57.对称的二叉树](#####57对称的二叉树)  
[58.按之字顺序打印二叉树](#####58按之字顺序打印二叉树)     
[59.把二叉树打印成多行](#####59把二叉树打印成多行)  
[60.序列化二叉树](#####60序列化二叉树)  
[61.二叉搜索树的第k个节点](#####61二叉搜索树的第k个节点)  
[62.数据流的中位数](#####62数据流的中位数)  
[63.滑动窗口的最大值](#####63滑动窗口的最大值)    
[64.矩阵中的路径](#####64矩阵中的路径)  
[65.机器人的运动范围](#####65机器人的运动范围)  
...
```
//思想就是区分第二个是不是*，不是好办，是的话比较复杂，难以进行条件分类，因为不知道*前面的字符要匹配多少个str的字符，
//可能模式的后面也是相同的，因此需要||两种情况，一种一个不匹配，模式直接向后移动两个、一种匹配一个str后移一个，模式不变然后重新来匹配。
//这样就能覆盖所有情况了，不然实在太复杂。
//9.23号看的感触：对于.只要看成是匹配的就好了。对于*如果当前字符是匹配的，且下一个为*，直接return向后移动两个和不移动的||就好了
public class Solution {
...
```
//思想：这道题不需要使用递归，按顺序遍历就行，只关注特殊符号，数字单独使用一个函数过滤。
//然后特殊符号遵循[sign]intergral-digits[.[fractional-digits]][e|E[sign]exponential-digits]，其中[]表示不一定有，intergral-digits表示数字sign表示正负号
//还有一种使用正则表达式的写法,如下
/*
public class Solution {
...
*/
//9.23重温：一个index，从头遍历到尾，按顺序+-.e+-中间过滤数字一步一步来，如果不符合这个就是错的。
public class Solution {
    public boolean isNumeric(char[] str) {
        if(str == null){
            return false;
...
public class Solution {
	...
	// 注意: 题意是排序链表
	// 注意：每个return实际上都是连接调用栈上层的pHead.next，只有链表头结点，如果有重复元素，返回的是最后一个不重复节点。
...
public class Solution {
    ...
    //首先做一个规定，大顶堆和小顶堆依次获得元素。小顶堆最小的元素需要大于大顶堆的元素，这样两个堆顶元素才是数据流中最中间的两个数
    boolean flag = true;
    public void Insert(Integer num) {
        if(flag){//奇数个，需要将多余的添加到大顶堆
...
```
//思想：显然滑动窗口是队列实现的，然后建立一个表示滑动窗口最大值的双端队列。
//然后就是滑动窗口每次变动的时候考虑双端队列尾元素可能不可能是最大元素，不可能就删掉。添加当前元素到队尾。考虑队列头是否在范围内。取出当前队列头。
//每次循环的时候
//这道题要考虑的点很多，一一写下，看一看
import java.util.LinkedList;
import java.util.ArrayList;
public class Solution {
    public ArrayList<Integer> maxInWindows(int[] num, int size)
    {
        ...
        for(int i = 0; i < num.length; i++){
            //1、这个队列的性质是后边的元素小于前边的元素。如果大于的话就要剔除前边的元素，应该从后向前开始剔除，因为是队列是降序的。
            //双端队列
			//从双端队列来看，如果要添加进来的大于队尾，队尾一定不会是最大的了，所以去掉，双端队列前面大的可能是也可能后面会不符合条件去掉。
...
public class Solution {
    ...
    private boolean hasPathCore(char[] matrix, int rows, int cols, char[] str, int i, int j, int k, int[] flag){
        ...
        //不会出现反复进入同一个节点的情况，因为上一层是有序的调用上下左右。上不合适就下，下不合适就左。。。。。。直到合适或者不合适再跳到上一层。然后当前点还是可以进入的，在字符串后面如有相同字符。
        flag[index] = 0;
        return false;
    }
}
